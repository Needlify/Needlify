...
import TimeElapsed from "../components/vue/TimeElapsed.vue";

describe("Import vue component", () => {
    test("normal imports as expected", async () => {
        const cmp = await import("../components/vue/TimeElapsed.vue");
        expect(cmp).toBeDefined();
    });

    test("dynamic imports as expected", async () => {
        const name = "TimeElapsed";
        const cmp = await import(`../components/vue/${name}.vue`);
        expect(cmp).toBeDefined();
    });
});
